...
```{r}
total_global_earnings <- colSums(all_harry_potter_matrix)
```

2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
more_all_harry_potter_matrix <- rbind(all_harry_potter_matrix, total_global_earnings)
more_all_harry_potter_matrix
```
