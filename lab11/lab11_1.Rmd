...
homerange %>% 
  ...
  geom_histogram(alpha = 0.2, color = "black", fill = "darkgoldenrod2",bins=40)+
...
homerange%>%
  ...
  geom_histogram(alpha=.5, color="black", fill="blue", bins=20)+
...
homerange%>%
  ggplot(aes(x=log10.hra))+
  geom_density(alpha=.5, color="black", fill="blue")+
  labs(title="Distribution of hra")
```
...
homerange%>%
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=..density..), alpha=.5, color="black", fill="blue", bins=20)+
...
homerange%>%
  ...
  select(taxon, class, log10.mass, log10.hra, range_category)
```
